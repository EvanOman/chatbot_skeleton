...
class ThreadResponse(BaseModel):
    ...
    status: ThreadStatus = Field(..., description="Current status of the thread")
...
class MessageResponse(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of message", examples=["text", "file", "image"]
    )
    metadata: dict[str, Any] = Field(
        default_factory=dict,
...
class ErrorResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [
                {"error": "NotFound", "detail": "Thread not found"},
                {"error": "ValidationError", "detail": "Invalid UUID format"},
            ]
        }
    }
