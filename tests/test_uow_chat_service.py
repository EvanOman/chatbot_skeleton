...
from src.application.services.uow_chat_service import UowChatService, SYSTEM_USER_ID
...
class MockChatRepository(ChatRepository):
    ...
    async def insert_message(
        ...
        *, 
        ...
        if client_msg_id:
            existing = [m for m in self.messages if m.get("client_msg_id") == client_msg_id]
            if existing:
...
class TestUowChatService:
    ...
    @pytest.mark.asyncio
    async def test_add_message_and_reply(self, chat_service, mock_repo, mock_llm_generate):
