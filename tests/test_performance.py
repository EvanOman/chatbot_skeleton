...
class TestResponseTimes:
    ...
    @pytest.mark.asyncio
    async def test_api_response_times(self, async_client: AsyncClient, performance_timer, test_user_id: UUID):
        """Test that API endpoints respond within acceptable time limits."""
        
        # Test thread creation time
        performance_timer.start()
        response = await async_client.post("/api/threads/", json={
        ...
        assert response.status_code == 200
        performance_timer.assert_under(1.0)  # Should retrieve messages in under 1 second

    def test_static_endpoint_performance(self, test_client: TestClient, performance_timer):
    ...
    @pytest.mark.asyncio
    async def test_export_performance(self, async_client: AsyncClient, performance_timer):
        """Test export functionality performance."""
        # Create a thread with multiple messages first
        thread_response = await async_client.post("/api/threads/", json={
            "user_id": str(uuid4()),
...
class TestConcurrentRequests:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_message_sending(self, async_client: AsyncClient, test_user_id: UUID):
        ...
        async def send_message(message_num: int):
            response = await async_client.post(
                f"/api/threads/{thread_id}/messages",
                json={"content": f"Concurrent message {message_num}", "message_type": "text"},
                params={"user_id": str(test_user_id)}
            )
            return response.status_code == 200
...
class TestAgentPerformance:
    ...
    async def test_memory_system_performance(self, performance_timer):
        ...
        performance_timer.assert_under(5.0)  # Should store 100 memories in under 5 seconds
        ...
        performance_timer.assert_under(2.0)  # Search should be fast even with many memories
...
class TestDatabasePerformance:
    ...
    @pytest.mark.asyncio
    async def test_bulk_thread_creation(self, db_session: AsyncSession, performance_timer):
        ...
        performance_timer.assert_under(5.0)  # Should create 50 threads in under 5 seconds

    @pytest.mark.asyncio
    async def test_bulk_message_creation(self, db_session: AsyncSession, performance_timer):
        ...
        performance_timer.assert_under(10.0)  # Should create 100 messages in under 10 seconds

    @pytest.mark.asyncio
    async def test_message_query_performance(self, db_session: AsyncSession, performance_timer):
        ...
        performance_timer.assert_under(2.0)  # Should query 200 messages in under 2 seconds
...
class TestMemoryUsage:
    ...
    async def test_large_conversation_memory(self, async_client: AsyncClient):
        ...
        for i in range(50):
            await async_client.post(
                f"/api/threads/{thread_id}/messages",
                json={
                    "content": f"This is a longer test message {i} " * 10,  # Make messages longer
                    "message_type": "text"
                },
                params={"user_id": str(uuid4())}
            )
        ...
        assert memory_increase < 100, f"Memory increased by {memory_increase:.2f}MB, which seems excessive"
    ...
    def test_agent_memory_cleanup(self):
        ...
        assert memory_increase < 200, f"Memory increased by {memory_increase:.2f}MB after agent cleanup"
...
class TestStressTests:
    ...
    async def test_rapid_api_requests(self, async_client: AsyncClient):
        ...
        assert len(successful_responses) >= 15, "Too many requests failed under rapid load"
        
        # Should complete in reasonable time
        total_time = end_time - start_time
        assert total_time < 10, f"Rapid requests took {total_time:.2f}s, which is too slow"

    @pytest.mark.asyncio
    async def test_large_message_handling(self, async_client: AsyncClient, test_user_id: UUID):
        ...
        assert len(user_messages[0]["content"]) > 10000  # Verify large content was stored
